...
class ARHSettingsC(Structure):
    _fields_ = [("logger", c_void_p), ("initialized", c_bool), ("verbose", c_int)]
...
def arh_factory(
    ...
    get_fock: Callable[[np.ndarray, np.ndarray], float],
    settings: Settings,
) -> Tuple[
    ...
    update_orbs_arh_interface.get_fock_interface = get_fock_interface
