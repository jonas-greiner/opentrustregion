...
_Static_assert(offsetof(solver_settings_type, precond_pd) == 1 * sizeof(void*),
...
_Static_assert(offsetof(stability_settings_type, init_trial_space) == 3 * sizeof(void*),
...
bool test_solver_settings_init(void)
{
    ...
    if (ss.hess_symm != stability_defaults.hess_symm) {
        fprintf(stderr,
                "test_solver_settings_init failed: Nested stability Hessian symmetry "
                "parameter wrong.\n");
        ok = false;
    }
    if (ss.stop_on_instability != stability_defaults.stop_on_instability) {
        fprintf(stderr,
                "test_solver_settings_init failed: Nested stability stop-on-"
                "instability parameter wrong.\n");
        ok = false;
    }
    ...
    if (ss.seed != stability_defaults.seed) {
        fprintf(stderr,
                "test_solver_settings_init failed: Nested stability seed "
                "parameter wrong.\n");
        ok = false;
    }
    ...
    if (strcmp(ss.diag_solver, stability_defaults.diag_solver) != 0) {
        fprintf(stderr,
                "test_solver_settings_init failed: Nested stability diagonal "
    ...
    if (ss.precond || ss.project || ss.approx_hess_x || ss.init_trial_space ||
        ...
        fprintf(stderr,
                "test_solver_settings_init failed: Nested stability callback "
...
bool test_stability_check_c(void)
{
    ...
    c_bool stable = false;
    c_real min_eigval = 0.0;
    c_int error = stability_check(h_diag, hess_x_fun, N_PARAM, &stable, &settings,
                                  direction, &min_eigval);
    if (min_eigval < 0.0) {
        fprintf(stderr, "test_stability_check_c failed: Lowest eigenvalue at minimum "
    ...
    error = stability_check(h_diag, hess_x_fun, N_PARAM, &stable, &settings, direction,
                            &min_eigval);
    if (min_eigval >= 0.0) {
        fprintf(stderr, "test_stability_check_c failed: Lowest eigenvalue at saddle "
